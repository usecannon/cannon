...
export const InteractTab: FC<{
  ...
  const [isProcessingNav, setIsProcessingNav] = useState(false);
  ...
  return (
    <SubnavContext.Provider value={{ hasSubnav: true }}>
      ...
      {/* {deploymentData.isLoading ||
        ...
        (isProcessingNav && <CustomSpinner />)} */}
      {deploymentData.isLoading ||
        packagesQuery.isLoading ||
        (isProcessingNav && <Box>LOADINGGGGG </Box>)}
      {routing && <Box>ROUTINGGGGG </Box>}

      {/* {isProcessingNav && <CustomSpinner />}
      {isProcessingNav && <Box>LOADINGGGGG USEE </Box>} */}
