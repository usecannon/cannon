      ...
      try {
        ...
        simulation.on('tick', () => {
          ...
          arrowLines
            ...
            .attr(
              'x2',
              (d: any) => d.source.x + (d.target.x - d.source.x) * 0.5
            ) // Corrected midpoint x
            .attr(
              'y2',
              (d: any) => d.source.y + (d.target.y - d.source.y) * 0.5
            ); // Corrected midpoint y

          // Update node and text positions
          node.attr('transform', (d) => `translate(${d.x}, ${d.y})`);
        });

        // Define zoomToFit function at the top level
        const zoomToFit = () => {
          ...
          const scale = Math.min(
            0.8 / Math.max(dx! / width!, dy! / height!),
            4
          );
