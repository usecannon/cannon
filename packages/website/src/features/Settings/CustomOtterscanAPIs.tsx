...
export default function CustomOtterscanAPIs() {
  const [inputValue, setInputValue] = useState('');
  const [inputError, setInputError] = useState<string | undefined>();

  const settings = useStore((s) => s.settings);
  const setSettings = useStore((s) => s.setSettings);
  const { otterscanApis } = useCannonChains();
  ...
  const addProvider = () => {
    ...
    setSettings({
      customOtterscanAPIs: [...settings.customOtterscanAPIs, inputValue],
    });
    setInputValue('');
  };

  const removeProvider = (index: number) => {
    const updatedProviders = [...settings.customOtterscanAPIs];
    updatedProviders.splice(index, 1);
    setSettings({ customOtterscanAPIs: updatedProviders });
  };
  ...
  const findChainIdByUrl = (targetUrl: string) => {
    ...
    for (const id in otterscanApis) {
      if (otterscanApis[id].rpcUrl === targetUrl) {
        return id;
      }
    }
    return undefined;
  ...
  return (
    <div className="space-y-2">
      <Label>Otterscan API URLs</Label>

      <div className="space-y-2">
        {settings.customOtterscanAPIs.map((url, index) => (
          <div
            key={index}
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          <Input
            placeholder="e.g. https://mainnet.otterscan.io/"
            value={inputValue}
      ...
      <SettingResetButton
        onReset={() =>
          setSettings({
            customOtterscanAPIs: initialState.settings.customOtterscanAPIs,
          })
        }
        sectionName="Custom Otterscan APIs"
      />
    </div>
  );
}
