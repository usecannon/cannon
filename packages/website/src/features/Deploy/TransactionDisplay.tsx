...
>
  ...
  <Box ml={2.5}>
    <AlertTitle lineHeight={1} fontSize="sm" mb={1.5}>GitOps Deployment</AlertTitle>
    <AlertDescription display="block" lineHeight={1} fontSize="sm">These transactions were generated by <Link onClick={onOpen}>modifying {Array.from(cannonDefInfo?.filesList || [])?.length} cannonfiles</Link> in <Link isExternal href={gitUrl}>this repository</Link>.</AlertDescription>
  </Box>
</ChakraAlert>}
      ...
      {props.verify ? (
        <Box mb={8}>
          <Button
            ...
            as="a"
            mb={8}
            href={`https://dashboard.tenderly.co/simulator/new?block=&blockIndex=0&from=${props.safe.address
              }&gas=${8000000}&gasPrice=0&value=${props.safeTxn?.value
              }&contractAddress=${props.safe?.address
              }&rawFunctionInput=${createSimulationData(props.safeTxn)}&network=${props.safe.chainId
