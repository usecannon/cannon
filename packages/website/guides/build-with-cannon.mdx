...
### Bonus Step: Simulate a build with `--dry-run`

Before deploying to a live network, you can simulate the build process using the `--dry-run` flag. This allows you to test your `cannonfile.toml` and estimate gas costs without sending any transactions or making any changes to the blockchain.

When using `--dry-run`, you must also specify which signers to impersonate using either the `--impersonate <addresses>` or `--impersonate-all` flag. This ensures that the simulation can execute all the necessary transactions as if they were signed by the appropriate accounts.

To run a dry run of your build, use the following command:

```bash
cannon build --dry-run --impersonate-all
```

The output will be similar to a regular build, but it will clearly state that it's a simulation and no changes will be made. It will also provide an estimated total cost for the deployment.
