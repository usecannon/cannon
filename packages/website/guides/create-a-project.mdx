...
1. Edit your `cannonfile.toml` to include the following actions:
   ...
   *   The `[invoke.set_number]` action calls the `setNumber` function on the previously deployed `Counter` contract with the argument `420` (from `settings.number`). Similar to `deploy`, `invoke` actions support a number of additional features and capabilities which can be read in the docs.
