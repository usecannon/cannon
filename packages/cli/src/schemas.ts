...
export const runSchema = z.object({
  ...
  args: z.array(z.string()).describe('Arguments passed to the function (after the ChainBuilder object)').optional(),
  /** Environment variables to be set on the script */
  env: z.array(z.string()).describe('Environment variables to be set on the script').optional(),
  /** List of operations that this action depends on */
  depends: z
    .array(
      z.string().refine((val) => Boolean(val.match(stepRegex)), {
        message: `Must reference another action in the cannon package. For example: 'contract.Storage'`,
      }),
    )
    .describe(
