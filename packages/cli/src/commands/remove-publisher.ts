...
export async function removePublisher({ cliSettings, options, packageRef, publisher }: Params) {
  ...
  // throw an error if the address is not already a publisher
  ...
  if (!isExistingPublisher) {
    throw new Error(`The address "${publisher}" isn't a publisher for "${packageName}" package.`);
  }

  const confirm = await prompts({
    type: 'confirm',
    name: 'confirmation',
    message: `Do you want to remove ${publisher} as a publisher?`,
  });
