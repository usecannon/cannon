...
export async function interact(ctx: InteractTaskArgs) {
  ...
  while (true) {
    if (!pickedContract) {
      pickedContract = await pickContract({ contractNames: _.keys(ctx.contracts) });
      ...
      pickedFunction = await pickFunction({ contract: ctx.contracts[pickedContract] });
...
async function query({
  ...
  let result = [];
  try {
    result = await contract.callStatic[functionSignature!](...args, { blockTag });
  } catch (err) {
