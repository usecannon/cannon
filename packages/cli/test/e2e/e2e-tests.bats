#!/usr/bin/env bats

# Runs once before first setup
setup_file() {
  export CANNON_DIR="$(git rev-parse --show-toplevel)"
  export CANNON="node $CANNON_DIR/packages/cli/bin/cannon.js"
}

# Test pre-hook
setup() {
    load './helpers/bats-support/load'
    load './helpers/bats-assert/load'

    # get the containing directory of this file
    # use $BATS_TEST_FILENAME instead of ${BASH_SOURCE[0]} or $0,
    # as those will point to the bats executable's location or the preprocessed file respectively
    DIR="$( cd "$( dirname "$BATS_TEST_FILENAME" )" >/dev/null 2>&1 && pwd )"

    # make executables in scripts/ visible to PATH so tests can run files without relative path
    PATH="$DIR/scripts:$PATH"

    # Create temporary directory for tests
    export WORKDIR="$(mktemp -d)"
    export CANNON_DIRECTORY="$WORKDIR/cannondir"
    cd $WORKDIR
}

# Test post-hook
teardown() {
  if [ -e $WORKDIR]; then
    rm -rf $WORKDIR
  fi
}

@test "alter: import contract" {
   run alter-import-contract.sh
   echo $output
   assert_output --partial 'ipfs://QmQZu9RscJYaiwqN2qEhkpEVGTFKPj6K54yV7WnX7CPKAt'
}

@test "alter: import invoke" {
   run alter-import-invoke.sh
   echo $output
   assert_output --partial 'ipfs://QmQZu9RscJYaiwqN2qEhkpEVGTFKPj6K54yV7WnX7CPKAt'
}

@test "Build synthetix-v3 repo" {
   run downstream-ci-synthetix-v3.sh
   echo $output
   assert_output --partial 'buyback-snx:3.3.4@main built for Cannon (Chain ID: 13370)'
   assert_output --partial 'op-gas-price-oracle:3.3.4@main built for Cannon (Chain ID: 13370)'
   assert_output --partial 'pyth-erc7412-wrapper:3.3.4@main built for Cannon (Chain ID: 13370)'
   assert_output --partial 'spot-market-oracle:3.3.4@main built for Cannon (Chain ID: 13370)'
   assert_output --partial 'synthetix-legacy-market:3.3.4@main built for Cannon (Chain ID: 13370)'
   assert_output --partial 'oracle-manager:3.3.4@main built for Cannon (Chain ID: 13370)'
   assert_output --partial 'synthetix:3.3.4@main built for Cannon (Chain ID: 13370)'
   assert_output --partial 'synthetix-spot-market:3.3.4@main built for Cannon (Chain ID: 13370)'
   assert_output --partial 'synthetix-perps-market:3.3.4@main built for Cannon (Chain ID: 13370)'
}

@test "Build synthetix-deployments repo" {
   run downstream-ci-synthetix-deployments.sh
   echo $output
   assert_output --partial 'synthetix-omnibus:3.3.5@andromeda built on Base Mainnet (Chain ID: 8453)'
}

@test "build: building with foundry" {
   run build-foundry.sh
   echo $output
   assert_output --partial 'greeter-foundry:2.4.21@main built for Cannon (Chain ID: 13370)'
}

@test "build: building with hardhat" {
   run build-hardhat.sh
   echo $output
   assert_output --partial 'greeter:2.10.0@main built for Cannon (Chain ID: 13370)'
}

@test "publish: publishing package to a local network" {
   run publish.sh
   echo $output
}

@test "run: run package" {
   run run.sh
   echo $output
}

@test "verify: verify package" {
   run verify-build.sh
   echo $output
}
