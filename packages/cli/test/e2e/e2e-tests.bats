...
setup_file() {
  ...
  export CANNON_RPC_URL_OPTIMISM="${CANNON_RPC_URL_OPTIMISM:="https://optimism.gateway.tenderly.co"}"
  export CANNON_RPC_URL_ETHEREUM="${CANNON_RPC_URL_ETHEREUM:="https://mainnet.gateway.tenderly.co"}"

  # Fork OP to run tests against forked node
  anvil --fork-url "$CANNON_RPC_URL_OPTIMISM" --port 9546 --silent --accounts 1 --optimism &
  export ANVIL_PID_OP="$!"
  sleep 1

  # Fork Mainnet to run tests against forked node
  anvil --fork-url "$CANNON_RPC_URL_ETHEREUM" --port 9545 --silent --accounts 1 &
  export ANVIL_PID="$!"
  sleep 1
}
