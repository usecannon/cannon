...
async function dumpStorage(node: HardhatNode): Promise<{ [key: string]: any }> {
  ...
  if (node._stateManager._state) {
    ...
    // @ts-ignore
    const acc = await node._stateManager.getAccount(
...
async function createWithPersist(
  config: NodeConfig,
  state: SerializableNodeState
  ...
  if (isForkedNodeConfig(config)) {
    ...
    const forkStateManager = new ForkStateManager(forkClient, forkBlockNumber);
    ...
    const genesisBlockBaseFeePerGas = hardforkGte(hardfork, HardforkName.LONDON)
