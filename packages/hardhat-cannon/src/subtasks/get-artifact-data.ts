...
subtask(SUBTASK_GET_ARTIFACT).setAction(
  async ({ name }, hre): Promise<ContractArtifact> => {
    const art = (await hre.artifacts.readArtifact(name)) as ContractArtifact;
    if (art) {
      const dependencyGraph: any = await hre.run('compile:solidity:get-dependency-graph', { sourceNames: [art.sourceName] });
