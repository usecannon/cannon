...
task(
  ...
  .addOptionalParam(
    'preset',
    'Specify the preset label the given settings should be applied',
    'main'
  )
  ...
  .setAction(
    async ({ noCompile, file, options, dryRun, port, preset, wipe }, hre) => {
      if (!noCompile) {
      ...
      if (dryRun) {
        ...
        if (hre.network.name != 'hardhat')
          throw new Error(
            'Hardhat selected network must be `hardhat` in order to dryRun.'
          );
        ...
        if (!network)
          throw new Error(
            'Selected dryRun network not found in hardhat configuration'
          );

        if (!network.chainId)
          throw new Error(
            'Selected network must have chainId set in hardhat configuration'
          );
      ...
      for (const dependency of dependencies) {
        console.log(
          `Loading dependency tree ${dependency.source} (${dependency.chainId}-${dependency.preset})`
        );
      ...
      builder.on(Events.PreStepExecute, (t, n) =>
        console.log(`\nexec: ${t}.${n}`)
      );
      builder.on(Events.DeployContract, (n, c) =>
        console.log(`deployed contract ${n} (${c.address})`)
      );
      builder.on(Events.DeployTxn, (n, t) =>
        console.log(`ran txn ${n} (${t.hash})`)
      );
