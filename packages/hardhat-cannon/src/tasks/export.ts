...
task(TASK_EXPORT, 'Write a cannon chain from zip archive')
  ...
  .addOptionalParam(
    'chainLabel',
    'Name of the chain to export as it appears in hardhat. For example `mainnet` for mainnet.'
  )
  ...
  .setAction(
    async ({ file, packageName, packageVersion, chainLabel, preset }, hre) => {
      // if name, version not specified, resolve from cannonfile
      if (!packageName || !packageVersion) {
        const def = loadCannonfile(
          hre,
          path.join(hre.config.paths.root, 'cannonfile.toml')
        );
      ...
      if (chainLabel) {
        ...
        if (!newChainId) {
          throw new Error(
            `chain id for requested network ${chainLabel} not defined in hardhat configuration.`
          );
        }
      ...
      console.log(
        `Exported ${packageName}@${packageVersion} for network ${chainId}`
      );
    }
  );
