 ...
 */
export function makeArachnidCreate2(salt: string, initcode: ethers.utils.BytesLike, arachnidAddress = ARACHNID_CREATE2_PROXY): [ethers.providers.TransactionRequest, string] {
  let saltHash = salt;
  ...
  const contractAddress = '0x' + ethers.utils.keccak256(
    '0xff' + 
    arachnidAddress.slice(2) + 
    saltHash.slice(2) + 
    ethers.utils.keccak256(initcode).slice(2)
  ).slice(26);
