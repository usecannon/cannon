import { getSingletonFactoryInfo } from '@safe-global/safe-singleton-factory';
import Debug from 'debug';
import * as viem from 'viem';
import { CannonSigner, ChainBuilderRuntimeInfo } from './';

const debug = Debug('cannon:builder:create2');

export const ARACHNID_DEFAULT_DEPLOY_ADDR = '0x3fab184622dc19b6109349b94811493bf2a45362';
export const SAFE_SINGLETON_FACTORY_DEFAULT_DEPLOY_ADDR = '0xE1CB04A0fA36DdD16a06ea828007E35e1a3cBC37';

export const ARACHNID_DEPLOY_TXN =
  '';

/**
 * If create2 proxy is not already deployed, this function can deploy it.
 * @param runtime the runtime which can be used to deploy
 */
export async function ensureProxyCreate2Exists(
  runtime: ChainBuilderRuntimeInfo,
  deployer: viem.Address
): Promise<viem.Address> {
  // if create2 contract is not deployed
  const proxyAddress = viem.getCreateAddress({ from: deployer, nonce: BigInt(0) });
  const detectedBytecode = await runtime.provider.getCode({ address: proxyAddress });
  if (!detectedBytecode || detectedBytecode === '0x') {
    debug('create2 contract not found. attempting to deploy...');
    // on local testnets the contract is not deployed,
    // but we can deploy it easily

    // first "get" the signer (which will populate it for use and with enough eth for gas)
    // if signer doesn't exist then this isnt local testing network, and this txn will fail
    let signer: CannonSigner;
    try {
      signer = await runtime.getSigner(deployer);
    } catch (err) {
      debug('got deployer signer error', err);
      throw new Error(
        'could not populate deployer signer address. ' +
          'This most likely means you are trying to deploy to a non-local network, but the create2 contract has not been deployed. ' +
          'Please follow the instructions to deploy create2 proxy.'
      );
    }

    let data: viem.Hex =
      '0x604580600e600039806000f350fe7fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffe03601600081602082378035828234f58015156039578182fd5b8082525050506014600cf3';
    const info = getSingletonFactoryInfo(runtime.chainId);
    if (info && info.signerAddress === deployer) {
      // use safe singleton factory transaction
      data = info.transaction as viem.Hex;
    }

    // now run the presigned deployment txn
    const hash = await signer.wallet.sendTransaction({
      account: signer.address,
      chain: runtime.provider.chain,
      data,
    });

    await runtime.provider.waitForTransactionReceipt({ hash });
  }

  return proxyAddress;
}

/**
 * Returns the address generated by a CREATE2 call using the proxy
 */
export function makeProxyCreate2Txn(
  salt: string,
  initcode: viem.Hex,
  proxyAddress: viem.Address
): [Pick<viem.TransactionRequest, 'to' | 'data'>, viem.Address] {
  const saltHash: viem.Hash = !viem.isHash(salt) ? viem.keccak256(viem.toBytes(salt)) : (salt as viem.Hash);

  const txn = {
    to: proxyAddress,
    data: viem.concatHex([saltHash, initcode]),
  };

  const contractAddress = viem.getCreate2Address({ bytecode: initcode, salt: saltHash, from: proxyAddress });

  // viem.getAddress will uppercase the address properly for checksum purposes
  return [txn, viem.getAddress(contractAddress)];
}
