...
exports.renderTrace = exports.renderResult = exports.findContract = exports.handleTxnError = void 0;
const ethers_1 = require("ethers");
const consoleLog_1 = require("./consoleLog");
const CONSOLE_LOG_ADDRESS = '0x000000000000000000636f6e736f6c652e6c6f67';
function handleTxnError(artifacts, provider, err) {
        ...
        if (data) {
            try {
                const r = findContract(ctx, ({ address, abi }) => {
                    try {
                        new ethers_1.ethers.Contract(address, abi).interface.parseError(data);
                        return true;
                    }
                    catch (_) { }
                    return false;
...
function findContract(ctx, condition, prefix = '') {
    for (const name in ctx.contracts) {
        if (condition(ctx.contracts[name])) {
            return {
                name: prefix + name,
                contract: new ethers_1.ethers.Contract(ctx.contracts[name].address, ctx.contracts[name].abi)
            };
        }
    }
...
function parseErrorReason(contract, data) {
    if (data.startsWith(ethers_1.ethers.utils.id('Panic(uint256)').slice(0, 10))) {
        // this is the `Panic` builtin opcode
        const reason = ethers_1.ethers.utils.defaultAbiCoder.decode(['uint256'], '0x' + data.slice(10))[0];
    ...
    else if (data.startsWith(ethers_1.ethers.utils.id('Error(string)').slice(0, 10))) {
        // this is the `Error` builtin opcode
        const reason = ethers_1.ethers.utils.defaultAbiCoder.decode(['string'], '0x' + data.slice(10));
        return `Error("${reason}")`;
    }
    else if (contract) {
        try {
...
function parseFunctionData(ctx, contractAddress, input, output) {
    ...
    if (contractAddress.toLowerCase() == CONSOLE_LOG_ADDRESS) {
        ...
        parsedInput = 'log' + renderResult(ethers_1.ethers.utils.defaultAbiCoder.decode(consoleLog_1.ConsoleLogs[parseInt(input.slice(0, 10))], '0x' + input.slice(10)));
