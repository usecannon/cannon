...
exports.ChainDefinition = exports.validateChainDefinition = exports.ActionKinds = void 0;
...
const debug = (0, debug_1.default)('cannon:builder:definition');
/**
...
class ChainDefinition {
    constructor(def) {
         ...
         */
        this.getDependencyTree = lodash_1.default.memoize((node) => {
            ...
            return lodash_1.default.uniq(allDeps);
        });
        this.raw = def;
        const actions = [];
        for (const kind in exports.ActionKinds) {
        ...
        // do some preindexing
        this.allActionNames = lodash_1.default.sortBy(actions, lodash_1.default.identity);
        this.roots = new Set(this.allActionNames.filter((n) => !this.getDependencies(n).length));
        this.leaves = new Set(lodash_1.default.difference(this.allActionNames, lodash_1.default.chain(this.allActionNames)
    ...
    getName(ctx) {
        return lodash_1.default.template(this.raw.name)(ctx);
    }
    getVersion(ctx) {
        return lodash_1.default.template(this.raw.version)(ctx);
    }
    getConfig(n, ctx) {
        if (lodash_1.default.sortedIndexOf(this.allActionNames, n) === -1) {
            throw new Error(`getConfig step name not found: ${n}`);
        }
        return exports.ActionKinds[n.split('.')[0]].configInject(ctx, lodash_1.default.get(this.raw, n));
    }
    /**
    ...
    getState(n, runtime, ctx) {
        return __awaiter(this, void 0, void 0, function* () {
            const obj = yield exports.ActionKinds[n.split('.')[0]].getState(runtime, ctx, this.getConfig(n, ctx));
            if (!obj) {
                return null;
            }
            else {
    ...
    getRequiredImports(ctx) {
        ...
        return lodash_1.default.uniq(Object.values(this.raw.import).map((d) => ({
    ...
    getDependencies(node) {
        return (lodash_1.default.get(this.raw, node).depends || []);
    }
    get topologicalActions() {
        ...
        return lodash_1.default.uniq(actions);
    }
    checkAll() {
        const invalidSchema = (0, exports.validateChainDefinition)(this.raw);
    ...
    getStateLayers(actions = this.topologicalActions, layers = {}, layerOfActions = new Map(), layerDependingOn = new Map()) {
        ...
        for (const n of actions) {
            ...
            const action = lodash_1.default.get(this.raw, n);
            if (!action) {
            ...
            for (const dep of deps) {
                ...
                if (dependingLayer && dependingLayer !== attachingLayer) {
                    ...
                    layers[dependingLayer].depends = lodash_1.default.uniq([...layers[dependingLayer].depends, ...layers[attachingLayer].depends]);
    ...
    getPrintLinesUsed(n, layers = this.getStateLayers()) {
        return Math.max(layers[n].actions.length + 2, lodash_1.default.sumBy(layers[n].depends, (d) => this.getPrintLinesUsed(d, layers)));
    }
    printPartialTopology(n, layers = this.getStateLayers(), line = 0) {
        const layer = layers[n];
        let output = '';
        // print myself
        const width = lodash_1.default.maxBy(layer.actions, 'length').length;
        if (line === 0) {
            output = '┌─' + '─'.repeat(width) + '─┐';
        }
    ...
    printTopology(nodes = []) {
        ...
        if (!nodes.length) {
            // get leaf layers
            const layerArray = lodash_1.default.uniq(Object.values(layers));
    ...
    toJson() {
        return lodash_1.default.cloneDeep(this.raw);
    }
}
