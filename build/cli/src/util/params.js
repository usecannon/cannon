...
export function parsePackageRef(val) {
...
export function parsePackageArguments(val, result) {
...
export function parsePackagesArguments(val, result = []) {
    const packageMatch = val.match(packageRegExp);
    if (packageMatch) {
        const { name, version = 'latest' } = packageMatch.groups;
        const def = {
    ...
    if (settingMatch) {
        if (!result.length)
            throw new InvalidArgumentError('Missing package definition before setting');
        const { key, value } = settingMatch.groups;
